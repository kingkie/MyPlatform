...
namespace Yu3zx.DapperExtend
{
    ...
    ///  SqlServer组装查询
    /// </summary>
    public class SqlServerQuery<TEntity> : Query<TEntity> where TEntity : IEntity
    {
        ...
        protected override string SelectSql
        {
            get
            {
                var sqlStr = base.SelectSql;
                 ...
                 if (TopNum > 0)
                 {
                     sqlStr = string.Format("SELECT TOP {0} {1} FROM {2} {3} ", TopNum, base.SelectColumn, this._ModelDes.TableName, this.WhereSql);
                 }
