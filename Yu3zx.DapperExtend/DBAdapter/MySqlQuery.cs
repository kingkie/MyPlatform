...
namespace Yu3zx.DapperExtend
{
    ...
    public class MySqlQuery<TEntity> : Query<TEntity> where TEntity : IEntity
    {
        /// <summary>
        /// 查询语句
        /// </summary>
        ...
        private string PageSql
        {
            get
            {
                ...
                if (_PageModel != null)
                {
                    ...
                    sql = string.Format(@"SELECT {0} FROM {1} WHERE Id >=(SELECT Id FROM {2} {3} {4} LIMIT {5},1 ) LIMIT {6}", base.SelectColumn, this._ModelDes.TableName, this._ModelDes.TableName,WhereSql,OrderSql, _PageModel.RowBegin - 1,_PageModel.PageSize);
                }
                return sql;
            }
        }
